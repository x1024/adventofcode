...
# _down = lambda x, y: (x, y, DOWN)
# _down_reverse = lambda x, y: (x, SIDE_WIDTH - 1 - y, DOWN)
# _down_flip = lambda x, y: (y, x, DOWN)
# lambda x, y: _reverse(x), _reverse(y), DOWN

def reverse(coord):
  return SIDE_WIDTH - 1 - coord
...
def rot0(state): return state
def rot4(state): return state
def rot1(state): return rotate_clockwise(state)
def rot2(state): return rotate_clockwise(rotate_clockwise(state))
def rot3(state): return rotate_clockwise(rotate_clockwise(rotate_clockwise(state)))
...
# print(rot3(state))
# print(rot4(state))
# print(rot1(rot4(state)))
# assert (rot2(rot2(state))) == state
# print("ASD")
# exit()
SIDE_WIDTH = 4
SIDE_WIDTH = 50
...
def move(data, path):
  ...
  boards = [get_board(x, y) for (x, y) in offsets]
  # for board in boards:
  #   print('\n'.join(''.join(row) for row in board))
  #   print()
  # exit()

  # x = max(0, min(SIDE_WIDTH - 1, start[0] % SIDE_WIDTH))
  # y = max(0, min(SIDE_WIDTH - 1, start[1] % SIDE_WIDTH))
  # pos = (x, y)
  pos = (0, 0)
  n = len(data)
  m = len(data[0])
  ...
  for (steps, turn) in path:
    ...
    for i in range(steps):
      ...
      # while data[new_pos[0]][new_pos[1]] == ' ': # wrap around
