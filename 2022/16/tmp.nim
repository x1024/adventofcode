valves = File.read('input').lines.to_h do |line|
...
def go(time, one, two, score, seen, open, valves, move_one, &)
  ...
  if move_one
    ...
    possible << [one, open + [one]] unless open.include?(one) or valves[one][:rate].zero?
  else
    possible = [[one, open]]
  end
  ...
  possible.each do |one, open|
    if not open.include?(two) and valves[two][:rate] > 0
      go(time - 1, one, two, score + flow(open + [two], valves), seen, new, valves, move_one, &)
    end
