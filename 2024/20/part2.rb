...
def main(input, limit = 100)
  ...
  result = 0
  visited = bfs(data, start, finish)
  p visited
  for pos in visited.keys
    for pos2 in visited.keys
      next if pos == pos2

      dist = (pos - pos2).real.abs + (pos - pos2).imag.abs
      next unless dist <= 20
      ...
      savings = (v2 - v1).abs - dist
      next unless savings >= limit

      p [pos, pos2, savings, dist, v1, v2]
      result += 1
    end
  end
