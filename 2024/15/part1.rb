...
def checksum(data)
  data.keys.filter { |k| data[k] == 'O' }.map { |k| k.real * 100 + k.imaginary }.sum
end
...
def main(input)
  ...
  moves.each do |move|
    ...
    new_pos = robot
    boxes = 1
    boxes += 1 while data[new_pos + offset * boxes] == 'O'
    if data[new_pos + offset * boxes] == '#'
      p 'wall'
    else
      data[new_pos + offset * boxes] = 'O'
      data[robot] = '.'
      robot = new_pos + offset
      data[robot] = '@'
    end
    # pr(data)
    # gets
