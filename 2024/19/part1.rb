...
def check(design)
  # p design
  return false if $seen.include?(design)

  $seen.add(design)
  return true if design.empty?

  $towels.each do |towel|
    next if design.size < towel.size
    next if design[0...towel.size] != towel

    return true if check(design[towel.size..-1])
  end
  false
end
...
def main(input)
  ...
  designs.filter.with_index do |d, i|
    $seen = Set.new
    p [i, designs.size, d]
    check(d)
  end.size
