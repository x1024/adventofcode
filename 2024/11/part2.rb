...
def evolve(stone, times)
  ...
  if stone == 0
    ...
    s = stone.to_s
    res += evolve(s[0...s.length / 2].to_i, times - 1)
    res += evolve(s[s.length / 2..].to_i, times - 1)
  else
    res += evolve(stone * 2024, times - 1)
  end
  MEMO[key] = res
  res
end
...
def main(input)
  data = input.scan(/\d+/).map { |n| n.to_i }
  ...
  result = data.map { |n| evolve(n, 75) }.sum
  # p lines
  # p data
