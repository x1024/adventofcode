...
def main(input, limit)
  ...
  ss = data.map { |n| sequences(n, limit) }
  rng = (-9..9).to_a
  part2 = 0
  rng.product(rng).product(rng).product(rng).map(&:flatten).each do |key|
    r = ss.sum { _1[key] || 0}
    if r > part2
      p r
      part2 = r
...
def sequences(n, limit)
  ...
  (limit - 4).times do |i|
    ...
    last << diff
    # p [n % 10, diff, last, n]
    # p ['!!!!!!!', s, last, price] if last == [-1, 2, 0, 3]
    h2[last.dup] ||= price
    old = price
    n = secret(n)
  end
  # result.each do |key, val| h[key] << val end
  h2
end

limit = 2000
p main(input_sample, limit)
# p result
result = main(input_real, limit)
p result
require 'clipboard'
