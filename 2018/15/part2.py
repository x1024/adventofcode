...
def solve(data, power=3, include_casualties=False):
  n, m = len(data), len(data[0])
  power = { ELF: power, GNOME: 3 }
  casualties = 0
  ...
  while True:
    ...
    for i in range(len(pieces)):
      ...
      if not get_pieces(opp):
        ...
        # for row in data: print(''.join(row))
        score = moves * sum(hp.values())

        if include_casualties:
          return score, casualties
...
def solve_hard(input):
  ...
  while True:
    ...
    score, casualties = solve(data, power, include_casualties=True)
    print(power, casualties, score)
    if casualties == 0:
      return score
    power += 1
