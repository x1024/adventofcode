    ...
    def play(self, elf_death=False):
        ...
        while True:
            if self.round(elf_death=elf_death):
                break
            print(rounds + 1)
            for u in sorted(self.units, key=lambda u: u.position):
                if u.position[0] in [18, 19, 20, 21]:
                    print(u.team, u.position, u.hp)
            self.print()
            rounds += 1
        for unit in self.units:
            if unit.alive:
                print(unit)
        print(rounds)
        print(sum(unit.hp for unit in self.units if unit.alive))
        exit()
