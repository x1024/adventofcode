...
def solve(_input):
  ...
  # pprint.pprint(input)
  # print_board(board, min_x = 470, max_x=530, min_y=0, max_y=40)
  propagate((START_X, START_Y))
  # print_board(board, min_x=min_x, max_x=max_x, min_y=min_y, max_y=max_y)
  # print_board(board, min_x=min_x, max_x=max_x, min_y=0, max_y=max_y)
  water = sum(1 for (x, y), val in board.items() if val in [STILL_WATER] and x >= min_x and y >= min_y and x <= max_x and y <= max_y)
  still_water = sum(1 for (x, y), val in board.items() if val in [WATER] and x >= min_x and y >= min_y and x <= max_x and y <= max_y)
