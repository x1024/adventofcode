...
def test():
  assert solve("^WNE$")[0] == 3
  assert solve("^ENWWW(NEEE|SSE(EE|N))$")[0] == 10
  assert solve("^ENNWSWW(NEWS|)SSSEEN(WNSE|)EE(SWEN|)NNN$")[0] == 18
  assert solve("^WSSEESWWWNW(S|NENNEEEENN(ESSSSW(NWSW|SSEN)|WSWWN(E|WWS(E|SS))))$")[0] == 31
  # exit()
