...
def step(state, rules):
  newstate = collections.defaultdict(lambda: '.')
  keys = list(state.keys())
  for x in range(min(keys) - 4, max(keys) + 5):
    key = ''.join((state[x-2], state[x-1], state[x], state[x+1], state[x+2]))
...
def solve(state, rules, steps=20):
  ...
  while i < steps:
    ...
    state_str = ''.join([state[i] for i in range(min(keys), max(keys) + 1)])
  ...
  res = sum(k + offset for k,v in state.items() if v == '#')
  return res

def parse_line(input):
  return [cell for cell in input.split(' => ')]


def parse_input(input):
  input = input.split('\n')
  state = input[0].replace('initial state: ', '')
  initial = collections.defaultdict(lambda: '.')
  initial.update(enumerate(state))
  rules = input[2:]
  rules = [row.strip() for row in rules]
  rules = [row for row in rules if row]
  # rules = list(map(int, rules))
  rulesDict = collections.defaultdict(lambda: '.')
  rules = list(map(parse_line, rules))
  # print(rules)
  rulesDict.update(rules)
