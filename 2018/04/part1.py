...
def solve(input):
  ...
  for id in data:
    # print(id)
    total_sleep = sum([sum(d.values()) for d in data[id].values()])
    if total_sleep > most_sleep:
      most_sleep = total_sleep
      most_sleep_id = id

  sleeps = collections.Counter()
  for day in data[most_sleep_id]:
    sleeps.update(data[most_sleep_id][day])
    print(day)
  print(sleeps)
  minute = sleeps.most_common(1)[0][0]
  print(most_sleep_id, minute)
  return most_sleep_id * minute
